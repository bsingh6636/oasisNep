...
// import { Pricelist as mockPriceList } from '../../const';
...
const SearchBar = () => {
  ...
  const { getPrices, priceListAll } = useContext(MyContext)
      ...
      <div ref={searchRef} className="relative max-w-2xl mx-auto">
        ...
        <div className="relative">
          <input
            ...
            className={`w-full p-2 py-3 pl-12 pr-5 rounded-xl border ${darkMode
          ...
          {searchValue && (
            <button
              ...
              className={`absolute right-4 top-1/2 transform -translate-y-1/2 text-gray-400 ${darkMode ? 'hover:text-gray-300' : 'hover:text-gray-600'
                }`}
        ...
        {isFocused && searchedItem.length > 0 && (
          <div className={`absolute z-10 mt-2 w-full rounded-xl shadow-lg border overflow-hidden ${darkMode
              ...
              {searchedItem.slice(0, 5).map(item => {
                ...
                return (
                  <Link
                    to={`/prices/${item.Name}`}
                    key={item.id}
                    className={`block px-4 py-3 ${darkMode ? 'hover:bg-gray-700' : 'hover:bg-blue-50'
                      } transition-colors`}
                    onClick={() => {
                      setSearchValue('');
                      setIsFocused(false);
        ...
        {isFocused && searchValue.length > 1 && searchedItem.length === 0 && (
          <div className={`absolute z-10 mt-2 w-full rounded-xl shadow-lg border ${darkMode
            ? 'bg-gray-800 border-gray-700'
            : 'bg-white border-gray-100'
            } transition-colors duration-300`}>

...
export const PriceDetails = () => {
  ...
  const [choosedMonth , setChooseMonth] = useState(null)
  ...
  useEffect(() => {
    ...
    if (item) {
      ...
      if (item.plans) {
        ...
        setChooseMonth(firstMonthKey); setSelectedMonth(firstMonthValue);
      }
    }
  }, [object]);

  const handleMonthClick = (key, value) =>{
   setChooseMonth(key);  setSelectedMonth(value);
  }
  ...
  return (
    <div className="max-w-4xl mx-auto p-8 shadow-lg rounded-lg bg-gradient-to-br from-gray-50 to-gray-200">
      <div className="flex flex-col md:flex-row items-center">
       ...
       <FAQ choosedMonth={choosedMonth} details={details}/>
       <CommentSection/>
    </div>
  );
};
